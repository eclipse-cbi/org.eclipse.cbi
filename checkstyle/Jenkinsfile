...
pipeline {
  ...
  environment {
    ...
    ARTIFACT_ID = sh(
      script: 'xml sel -N mvn="http://maven.apache.org/POM/4.0.0" -t -v  "/mvn:project/mvn:artifactId" "pom.xml"',
      returnStdout: true
    )
    GROUP_ID = sh(
      script: 'xml sel -N mvn="http://maven.apache.org/POM/4.0.0" -t -v  "/mvn:project/mvn:groupId" "pom.xml"',
      returnStdout: true
    )
  }
  ...
  stages {
      ...
      steps {
        sh '''
          ...
          if [ "${DRY_RUN}" = true ]; then
            >&2 echo "DRY RUN: ${WORKSPACE}/mvnw \"${VERSIONS_MAVEN_PLUGIN}:set\" -DnewVersion=\"${RELEASE_VERSION}\" -DgenerateBackupPoms=false"
            ...
            "${WORKSPACE}/mvnw" "${VERSIONS_MAVEN_PLUGIN}:set" -DnewVersion="${RELEASE_VERSION}" -DgenerateBackupPoms=false
            ...
            if "${WORKSPACE}/mvnw" "${MAVEN_DEPENDENCY_PLUGIN}:list" | grep SNAPSHOT; then
              >&2 echo "ERROR: At least one dependency to a 'SNAPSHOT' version has been found from 'pom.xml'"
              >&2 echo "ERROR: It is forbidden for releasing"
              exit 1
            fi

            if grep SNAPSHOT "pom.xml"; then
              >&2 echo "ERROR: At least one 'SNAPSHOT' string has been found in 'pom.xml'"
              >&2 echo "ERROR: It is forbidden for releasing"
              exit 1
            fi
          fi
    ...
    stage('Display plugin/dependency updates') {
      steps {
        sh '''
          "${WORKSPACE}/mvnw" "${VERSIONS_MAVEN_PLUGIN}:display-plugin-updates"
          "${WORKSPACE}/mvnw" "${VERSIONS_MAVEN_PLUGIN}:display-dependency-updates"
        '''
      }
    }
    ...
    stage('Build') {
      steps {
        sh '"${WORKSPACE}/mvnw" clean verify'
      }
    }
      ...
      steps {
        sh '''
          if [ "${DRY_RUN}" = true ] && [ "${RELEASE_VERSION}" != "" ] && [ "${NEXT_DEVELOPMENT_VERSION}" != "" ]; then
            >&2 echo "DRY RUN: ${WORKSPACE}/mvnw deploy"
          else
            "${WORKSPACE}/mvnw" deploy
          fi
        '''
      }
    }
      ...
      steps {
        sshagent(['git.eclipse.org-bot-ssh']) {
          sh '''
            ...
            "${WORKSPACE}/mvnw" "${VERSIONS_MAVEN_PLUGIN}:set" -DnewVersion="${NEXT_DEVELOPMENT_VERSION}" -DgenerateBackupPoms=false
            if [ "${DRY_RUN}" = true ]; then
              >&2 echo "DRY RUN: git add --all"
