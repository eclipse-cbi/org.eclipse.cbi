...
public interface Signer {
	...
	/**
	 * Sign the given file. It will retry a given number of time and will
	 * wait for a given amount time between each time. Returns true if the file
	 * has been successfully signed (the file is then modified), false otherwise
	 ...
	 *            positive.
	 * @param intervalUnit
	 *            unit of time for the {@code retryTimer} argument. Must not 
	 *            be {@code null}.
	 ...
	 *             not writable)
	 */
	boolean sign(Path path, int maxRetry, int retryInterval, TimeUnit intervalUnit) throws IOException;
	 ...
	 *             not writable)
	 */
	boolean sign(Path path) throws IOException;
}