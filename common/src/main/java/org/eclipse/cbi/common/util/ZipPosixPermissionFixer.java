...
abstract class ZipPosixPermissionFixer {
	...
	public void fixEntries() throws IOException {
		...
		Optional<CentralDirectoryHeader> directoryHeader = zipReader
				.readCentralDirectoryHeader(firstCentralDireactoryHeader);
		...
		if (!directoryHeader.isPresent()) {
			...
			do {
				...
				long nextDirectoryHeaderPosition = zipReader.position(directoryHeader.get())
						+ directoryHeader.get().size();
	...
	private long firstDirectoryHeaderPositionZ64(final EndOfCentralDirectory eocdrp) throws IOException, ZipException {
		...
		if (z64eocdl.isPresent()) {
			Optional<Zip64EndOfCentralDirectory> zip64eocd = zipReader.readZip64EndOfCentralDirectory(
					z64eocdl.get().relativeOffsetOfTheZip64EndOfCentralDirectoryRecord().longValue());
	...
	static final class ZipReader {
		...
		@SuppressWarnings("unchecked")
		public Optional<Zip64EndOfCentralDirectoryLocator> readZip64EndOfCentralDirectoryLocator(long position)
				throws IOException {
			return (Optional<Zip64EndOfCentralDirectoryLocator>) readRecord(
					Zip64EndOfCentralDirectoryLocator.DEFINITION, position);
		}
		...
		private Optional<Record> doReadRecord(Class<? extends Record> recordClass, RecordDefinition rd, long position)
				throws IOException {
	...
	static final class CentralDirectoryHeader extends Record.Fowarding {
		 ...
		 * are not compatible, then this value will identify the host system on which
		 * the attributes are compatible. Software can use this information to determine
		...
		/**
		 * 0 - MS-DOS and OS/2 (FAT / VFAT / FAT32 file systems) 1 - Amiga 2 - OpenVMS 3
		 * - UNIX 4 - VM/CMS 5 - Atari ST 6 - OS/2 H.P.F.S. 7 - Macintosh 8 - Z-System 9
		 * - CP/M 10 - Windows NTFS 11 - MVS (OS/390 - Z/OS) 12 - VSE 13 - Acorn Risc 14
		 * - VFAT 15 - alternate MVS 16 - BeOS 17 - Tandem 18 - OS/400 19 - OS X
		 * (Darwin) 20 thru 255 - unused
		 *
		 * @author mbarbero
		 *
		 */
		enum Platform {
			MSDOS_OS2(0), AMIGA(1), OPEN_VMS(2), UNIX(3), VM_CMS(4), ATARI_ST(5), OS2_HPFS(6), MACINTOSH(7),
			Z_SYSTEM(8), CP_M(9), WINDOWS_NTFS(10), MVS(11), VSE(12), ACORN_RISC(13), VFAT(14), ALTERNATE_MVS(15),
			BEOS(16), TANDEM(17), OS_400(18), OSX(19), UNKNOWN(-1);
	...
	/**
	 * end of central dir signature 4 bytes (0x06054b50) number of this disk 2 bytes
	 * number of the disk with the start of the central directory 2 bytes total
	 * number of entries in the central directory on this disk 2 bytes total number
	 * of entries in the central directory 2 bytes size of the central directory 4
	 * bytes offset of start of central directory with respect to the starting disk
	 * number 4 bytes .ZIP file comment length 2 bytes .ZIP file comment (variable
	 * size)
	 *
	 */
	static final class EndOfCentralDirectory extends Record.Fowarding {
		...
		public static final Field NOTDWTSITCD = createLEField(Field.Type.UINT_16,
				"number of the disk with the start of the central directory");
		public static final Field TNOEITCDOTD = createLEField(Field.Type.UINT_16,
				"total number of entries in the central directory on this disk");
		public static final Field TNOEITCD = createLEField(Field.Type.UINT_16,
				"total number of entries in the central directory");
		public static final Field SOTCD = createLEField(Field.Type.UINT_32, "size of the central directory");
		public static final Field OOSOCDWRTTSDN = createLEField(Field.Type.UINT_32,
	...
	/**
	 * zip64 end of central dir locator signature 4 bytes (0x07064b50) number of the
	 * disk with the start of the zip64 end of central directory 4 bytes relative
	 * offset of the zip64 end of central directory record 8 bytes total number of
	 * disks 4 bytes
	 *
	 */
	static final class Zip64EndOfCentralDirectoryLocator extends Record.Fowarding {
		...
		public static final Field Z64EOCDLS = createLEField(Field.Type.UINT_32,
				"zip64 end of central dir locator signature");
		public static final Field NODWSZ64EOCD = createLEField(Field.Type.UINT_32,
				"number of the disk with the start of the zip64 end of central directory");
		public static final Field ROZ64EOCDR = createLEField(Field.Type.UINT_64,
	 ...
	 *
	 * zip64 end of central dir signature 4 bytes (0x06064b50) size of zip64 end of
	 * central directory record 8 bytes version made by 2 bytes version needed to
	 * extract 2 bytes number of this disk 4 bytes number of the disk with the start
	 * of the central directory 4 bytes total number of entries in the central
	 * directory on this disk 8 bytes total number of entries in the central
	 * directory 8 bytes size of the central directory 8 bytes offset of start of
	 * central directory with respect to the starting disk number 8 bytes zip64
	...
	static class Zip64EndOfCentralDirectory extends Record.Fowarding {
		...
		public static final Field SOZ64EOCDR = createLEField(Field.Type.UINT_64,
				"size of zip64 end of central directory record");
		public static final Field VMB = createLEField(Field.Type.UINT_16, "version made by");
		public static final Field VNTE = createLEField(Field.Type.UINT_16, "version needed to extract");
		public static final Field NOTD = createLEField(Field.Type.UINT_32, "number of this disk");
		public static final Field NOTDWTSOTCD = createLEField(Field.Type.UINT_32,
				"number of the disk with the start of the central directory");
		public static final Field TNOEITCDOTD = createLEField(Field.Type.UINT_64,
				"total number of entries in the central directory on this disk");
		public static final Field TNOEITCD = createLEField(Field.Type.UINT_64,
				"total number of entries in the central directory");
		public static final Field SOTCD = createLEField(Field.Type.UINT_64, "size of the central directory");
		public static final Field OOSOCDWRTTSDN = createLEField(Field.Type.UINT_64,
				"offset of start of central directory with respect to the starting disk number");
	 ...
	 * Local file header:
	 *
	 * local file header signature 4 bytes (0x04034b50) version needed to extract 2
	 * bytes general purpose bit flag 2 bytes compression method 2 bytes last mod
	 * file time 2 bytes last mod file date 2 bytes crc-32 4 bytes compressed size 4
	 * bytes uncompressed size 4 bytes file name length 2 bytes extra field length 2
	 * bytes
	 *
