...
public class ApacheHttpClientSignerTest {
	...
	public void testSignNullFile() throws IOException {
		ApacheHttpClientSigner signer = createLocalSigner("localhost", 8080);
		signer.sign(null);
	}
	...
	public void testSignOfflineServer() throws Exception {
		FileSystem fs = Jimfs.newFileSystem(Configuration.unix());
		ApacheHttpClientSigner signer = createLocalSigner("qwerty", 8080);
		Path path = TestUtils.createLoremIpsumFile(fs.getPath("fileToSign"), 10);
		signer.sign(path);
	}
	...
	public void testRetryNegativeTimes() throws Exception {
		...
		ApacheHttpClientSigner signer = createLocalSigner("qwerty", 8080);
		Path path = TestUtils.createLoremIpsumFile(fs.getPath("fileToSign"), 10);
		signer.sign(path, -1, 0, TimeUnit.SECONDS);
	}
	...
	public void testRetryNegativeInterval() throws Exception {
		...
		ApacheHttpClientSigner signer = createLocalSigner("qwerty", 8080);
		Path path = TestUtils.createLoremIpsumFile(fs.getPath("fileToSign"), 10);
		signer.sign(path, 5, -4, TimeUnit.SECONDS);
	}
	...
	public void testSignStandardFile() throws Exception {
		...
		try {
			ApacheHttpClientSigner signer = createLocalSigner("localhost", getPort(server));
			Path path = TestUtils.createLoremIpsumFile(fs.getPath("fileToSign"), 10);
			assertTrue(signer.sign(path));
			try (InputStream newInputStream = Files.newInputStream(path)) {
				assertArrayEquals("Signed!".getBytes(), TestUtils.readAllBytes(newInputStream));
			}
		} finally {
			server.stop();
		}
	...
	public void testSignRequest() throws Exception {
		...
		try {
			...
			assertTrue(signer.sign(path));
			try (InputStream newInputStream = Files.newInputStream(path)) {
				assertArrayEquals("Valid!".getBytes(), TestUtils.readAllBytes(newInputStream));
			}
		} finally {
			server.stop();
		}
	...
	public void testServerError() throws Exception {
		...
		try {
			ApacheHttpClientSigner signer = createLocalSigner("localhost", getPort(server));
			Path path = TestUtils.createLoremIpsumFile(fs.getPath("fileToSign"), 10);
			assertFalse(signer.sign(path));
			try (InputStream newInputStream = Files.newInputStream(path)) {
				// check that the file content did not change
				assertArrayEquals(Files.readAllBytes(path), TestUtils.readAllBytes(newInputStream));
			}
	...
	public void testRetryOnServerError() throws Exception {
		...
		try {
			ApacheHttpClientSigner signer = createLocalSigner("localhost", getPort(server));
			Path path = TestUtils.createLoremIpsumFile(fs.getPath("fileToSign"), 10);
			assertFalse(signer.sign(path, 3, 100, TimeUnit.MILLISECONDS));
			try (InputStream newInputStream = Files.newInputStream(path)) {
				// check that the file content did not change
				assertArrayEquals(Files.readAllBytes(path), TestUtils.readAllBytes(newInputStream));
			}
	...
	public void testRetryOnException() throws Exception {
		...
		ApacheHttpClientSigner signer = createLocalSigner("localhost", 8080);
		Path path = TestUtils.createLoremIpsumFile(fs.getPath("fileToSign"), 10);
		signer.sign(path, 3, 100, TimeUnit.MILLISECONDS);
	}
	...
	private static ApacheHttpClientSigner createLocalSigner(String host, int port) {
		//return new ApacheHttpClientSigner(URI.create("http://"+host+":"+port+"/signing-service"), new SystemStreamLog());
		return new ApacheHttpClientSigner(URI.create("http://"+host+":"+port+"/signing-service"), new NullMavenLog());
	}
