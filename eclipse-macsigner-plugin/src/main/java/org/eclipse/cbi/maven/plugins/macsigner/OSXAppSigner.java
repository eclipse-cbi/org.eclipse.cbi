...
public class OSXAppSigner {
	...
	private OSXAppSigner(Signer signer, boolean continueOnFail, Log log, int maxRetry, int retryInterval, TimeUnit retryIntervalUnit) {
		this.signer = signer;
		this.log = log;
	...
	public int signApplications(Path baseSearchDir, Set<PathMatcher> pathMatchers) throws MojoExecutionException {
		...
		try {
			...
			exceptionHandler.handleError("Error occured while signing OS X application (" + Strings.join(", ", pathMatchers) + ").", e);
		}
     ...
     * @throws MojoExecutionException
     */
    public boolean signApplication(Signer signer, Path appFolder) throws MojoExecutionException {
    	boolean ret = false;
    	Path zippedApp = null;
    	...
    	try {
            ...
            if (!signer.sign(zippedApp, maxRetry, retryInterval, retryIntervalUnit)) {
