...
public class UpdatePackPropertiesFile extends Task {
	...
	public static void main(String[] args) {
		...
		// String archiveName =
		// "/Users/davidw/work/testSigning/orbit-signed-noPackProperties.zip";
	...
	private File ensureNewDirectory(String destinationdirectory) {
		...
		if (tempDestDir.exists()) {
			...
			if (destinationdirectory.endsWith(FILE_SEPERATOR)) {
				destinationdirectory = destinationdirectory.substring(0,
						destinationdirectory.lastIndexOf(FILE_SEPERATOR));
			}
	...
	/**
	 * This method uses a simple heuristic to determine if signed. It simple looks
	 * for "ECLIPSE.SF" and all known variants.
	 * 
	 * I was thinking that should be improved someday, but I looked at how Ant
	 * implements their "signed selector" and turns out that they do the same thing
	 * (basically). See http://grepcode.com/file/repo1.maven.org/maven2/org
				...
				try (JarFile jarFile = new JarFile(destinationDir + parentDir + jarname)) {
					...
					while (jarentries.hasMoreElements()) {
						...
						if (entryname.endsWith("ECLIPSE.SF") || entryname.endsWith("ECLIPSEF.SF")
								|| entryname.endsWith("ECLIPSE_.SF") || entryname.endsWith("ECLIPSEF_.SF")) {
		...
		if (!testFile.exists()) {
			...
			while (zipentry != null) {
				...
				if (zipentry.isDirectory()) {
					...
					// ZipEntries need not have the "directories" as unique entries in the zip file,
					// so we need to
					// infer if the full name contains any "new" directory segments that need to be
					// created.
					// If they do not exist, then "new FileOutputStream(newFile)" will throw a fit,
					// that is, a FileNotFound exception.
	...
	/*
	 * In addition to getting the java property "java.io.tmpdir", we ensure the
	 * string ends with a slash, since the caller will be appending 'zip file name'
	 * to it.
	 */
